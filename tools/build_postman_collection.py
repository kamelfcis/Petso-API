...
def append_shared_api_folders(items):
    ...
    ecom_items = [
        ...
        req(
            ...
            desc="Adds a product to the cart. If the product is already in the cart, quantity is incremented.",
        ),
        req(
            "Cart - Remove item",
            "POST",
            "/ecommerce/cart/remove_item/",
            {"product_id": "{{product_id}}"},
            desc="Removes the product entirely from the cart. Returns the updated cart.",
        ),
        req(
            "Cart - Update item quantity",
            "POST",
            "/ecommerce/cart/update_item/",
            {"product_id": "{{product_id}}", "quantity": 3},
            desc="Sets the quantity of a cart item to the given value. Pass `quantity=0` to remove the item.",
        ),
